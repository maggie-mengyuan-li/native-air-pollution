...
# mod.lmer <- lmer(annual_mean_all ~ county_type + year +
#                     popd_q + 
#                     hhinc_q +
#                     county_type*year +
#                     (1|State/County),
#                   data = dta.all, REML=FALSE)
# summary(mod.lmer)
...
## ADJUSTED PM VALUES! AT LAST!
monitor_adj <- pred_df %>% 
   modelr::add_predictions(mod.all$gam, interval = "confidence")

predict(mod.all$gam, newdata = pred_df, interval = "confidence")

## edit year column to just be last 2 characters
monitor_adj$year <- substr(monitor_adj$year, 3, 4)
...
# ## combine all dfs into one df; first select relevant cols only from each; NOT WORKING
# 
# native_exp_final <- native_exp_final %>% 
#   dplyr::select(year, county_type, State_Name, annual_mean_all) %>% 
#   mutate(State_Name = "AI-populated County State Mean Concentrations (Unadjusted)")
# 
# nonnative_exp_final <- nonnative_exp_final %>% 
#   dplyr::select(year, county_type, State_Name, annual_mean_all) %>% 
#   mutate(State_Name = "Non-AI-populated County State Mean Concentrations (Unadjusted)")
# 
# native_avgdec <- native_avgdec %>% 
#   dplyr::select(year, county_type, State_Name, annual_mean_all)
# 
# nonnative_avgdec <- nonnative_avgdec %>% 
#   dplyr::select(year, county_type, State_Name, annual_mean_all)
# 
# ai_monitor_adj <- ai_monitor_adj %>% 
#   dplyr::select(year, county_type, State_Name, annual_mean_all)
# 
# nai_monitor_adj <- nai_monitor_adj %>% 
#   dplyr::select(year, county_type, State_Name, annual_mean_all)
# 
# combined_monitor_df <- rbind.data.frame(native_exp_final, nonnative_exp_final,
#                                         native_avgdec, nonnative_avgdec,
#                                         ai_monitor_adj, nai_monitor_adj)
# combined_monitor_df
...
#             aes(x=year, y = annual_mean_all,
#                 group = State_Name, colour= State_Name),
#             color = "#e7d4e8") + #each line = state mean
#   geom_line(data = nonnative_exp_final,
#             aes(x=year, y = annual_mean_all,
#                 group = State_Name),
#             color = "#d9f0d3") + #unadjusted means by state
#   geom_line(data=native_avgdec,
#             aes(x=year, y=annual_mean_all,
#                 group=State_Name),
#             color="#af8dc3", linetype = "dashed", 
#             size = 1.25) + #unadjusted national mean (dashed)
#   geom_line(data=nonnative_avgdec,
#             aes(x=year, y=annual_mean_all,
#                 group=State_Name),
#             color="#7fbf7b", linetype = "dashed", 
#             size = 1.25) + #unadjusted national mean
#   geom_line(data= ai_monitor_adj,
#             aes(x=year, y=pred,
#                 group=State_Name),
...
# add caption/title
grob_monitor_combined <- grobTree(textGrob("Measured", x=0.05,  y=0.9, hjust=0,gp=gpar(fontsize=24)))

# Plot & save
monitor_combined + annotation_custom(grob_monitor_combined)


ggsave("figures/updated_10.22/monitor_combined.png")
...
# add caption/title
grob_model_combined <- grobTree(textGrob("Modeled", x=0.05,  y=0.9, hjust=0,gp=gpar(fontsize=24)))

# Plot & save
model_combined + annotation_custom(grob_model_combined)


ggsave("figures/updated_10.22/model_combined.png")
```
### Calulate average decline across study period (monitor and model) -- reported in results
```{r}
ai_monitor_adj
nai_monitor_adj
