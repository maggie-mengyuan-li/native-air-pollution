...
## Same thing with alternative model referent is county_type = 1 
## Duplicate df with all annual averages data to use
dta.all.ref1 <- all_ctyear_exp
mod.all.ref1 <- gamm4(annual_mean_all ~ county_type*as.factor(year) + popd_q + hhinc_q,
                 random =~(1|State/County),
                 data = dta.all.ref1)
summary(mod.all.ref1$gam)
...
dta.all$preds.r <- predict(mod.all$gam, dta.all, type = "terms")[,2] + # year main effect
                   predict(mod.all$gam, dta.all, type = "terms")[,5] # year*county_type interx
summary(predict(mod.all$gam, dta.all, type = "terms"))

# dta.no.ref <- all_ctyear_exp
# 
# mod.all.no.ref <- gamm4(annual_mean_all ~ county_type*as.factor(year) + popd_q + hhinc_q,
#                  random =~(1|State/County),
#                  data = dta.all)
# 
# summary(predict(mod.all.no.ref$gam, dta.no.ref, type = "terms"))
# summary(predict(mod.all$gam, dta.all, type = "terms"))
...
pm.ai.int  <- summary(mod.all.ref1$gam)$p.coeff[1] + ai.popd + ai.hinc 
...
ggsave("./figures/updated_9.27/monitor_ai_states_3.png")
```
...
ggsave("./figures/updated_9.27/monitor_nai_states_3.png")
