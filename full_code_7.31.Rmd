...
##3.4 State Line Graphs showing PM2.5 declines (monitor/model); 3 year Moving Averages
...
# # have a new_data for mod2 but need to aggregate across all years for the confounders
# new_data_mod2 <- aggregate(cbind(popd_q, hhinc_q)~year, data = monitor_ai_mod2_df, FUN = mean)
# new_data_mod2
...
adjPred 
# write out adjPred as csv for Marianthi//read it in
# setwd("/Users/maggieli/Dropbox/Native Air Pollution Paper/Paper/CodeCheck/native-air-pollution/Data/intermediate_data")
# write_csv(adjPred,
#           "adjPred.csv")
# adjPred <- read_csv("adjPred.csv")
# adjPred <- adjPred %>% dplyr::rename(PM = year) #rename year column to PM bc these are the differences in PM levels year to year

# Check that attr(adjPred, "constant") and mean(native_ctyear_exp$annual_mean_all) are the same; they are not D:
# adjPred 
# attr(adjPred, "constant")
# mean(native_ctyear_exp$annual_mean_all)
# attr(adjPred, "constant") == mean(native_ctyear_exp$annual_mean_all)
...
y <- 2 + 0.5*x + 0.1*z + rnorm(1000, 0, 3)

mod1 <- lm(y ~ x + z)
summary(mod1)

preds <- predict(mod1, type = "terms")
head(preds)
attributes(preds)$constant #one way to call overall mean
attr(preds, "constant") #another way to call overall mean
preds$constant
mean(y)

xcat <- cut(x, quantile(x, seq(0,1,0.1)), include.lowest = TRUE)
class(xcat)

mod2 <- lm(y ~ xcat + z)
summary(mod2)

preds2 <- predict(mod2, type = "terms")
head(preds2)

attr(preds2, "constant")

plot((preds2[,1]+mean(y)), (preds[,1]+mean(y)))
```
