...
### 8/21 update: plotting annual decline, not 3 year moving averages
...
# write_csv(monitor_ai_mod2_df, "monitor_ai_df.csv")

# Step 2: Run lm to get all the beta values
monitor_ai_mod2 <- lm(annual_mean_all ~ year + popd_q + hhinc_q, data = monitor_ai_mod2_df)
summary(monitor_ai_mod2)

# Step 3: Run a predict to get adjusted prediction values based on these betas
adjPred <- predict(monitor_ai_mod2, type = "terms")
adjPred <- as.data.frame(adjPred) #save matrix as dataframe
adjPred #each entry is the deviation from the overall mean for each year; it repeats every 18 entries to reflect the 19 different study years (1 is referent year 2000 so that is not an entry)
...
annual_adjPred <- annual_adjPred %>% mutate(adj_mean = preds + overall_mean) #column that adds global mean to predicted diffs
adjPred 
annual_adjPred 

# Step 4b: Need to move 19th row of annual_adjPred to be 1st row because this actually represents the annual deviation of 2000 values from the overall mean (unique function does not work perfectly)
annual_adjPred <- annual_adjPred %>% slice_tail() %>% #subset the last year row
  mutate(year = "00") %>% bind_rows(annual_adjPred) %>% slice(1:(n()-1)) %>% #bind last year row up to first row, delete last row
  mutate(year = new_data$year) #redo the year column so that it is in the correct order starting from 2000
...
annual_adjPred$State_Name <- "Adjusted Mean Concentration"
...
# Step 2: Run lm to get all the beta values
monitor_non_ai_mod2 <- lm(annual_mean_all ~ year + popd_q + hhinc_q, data = monitor_non_ai_mod2_df)
summary(monitor_non_ai_mod2)

# Step 3: Run a predict to get adjusted prediction values based on these betas
adjPred_non_ai <- predict(monitor_non_ai_mod2, type = "terms")
...
adjPred_non_ai
annual_adjPred_non_ai <- adjPred_non_ai %>% slice(55:73) #rows from FIPS 01049 entries; vector of 19 values for each study year
# annual_adjPred_non_ai <- data.frame("preds" = annual_adjPred_non_ai) #vector to dataframe
...
annual_adjPred_non_ai$State_Name <- "Adjusted Mean Concentration"
...
#PLOT OF TOTAL EFFECT OF NATIVE OVER TIME; updated 8/28
