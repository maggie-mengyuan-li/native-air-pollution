...
filter(PM2_5_1999_2018_rural_clipped,
       is.na(MF.Value)) #there are NAs now!
...
#clean up df to just only include sitecode, year, and days between measurements 
IMPROVE_interval <- IMPROVE_interval %>% subset(select = c(SiteCode,
                                                           Year,
                                                           Between))

#Compare IMPROVE_interval to number of total observations in each year (8/15/20 edit: NEED TO NA.OMIT ORIGINAL DATA; THE NUMBER OF OBSERVATIONS CANNOT INCLUDE OBSERVATION DAYS WITH NA FOR MF.VALUE FIELD)
...
# IMPROVE_check_interval %>% filter( == "AGTI1")
view(IMPROVE_check_interval)
...
# 8/15/20 update: code checking to see why there are different number of rows in PM25_rural_annual and IMPROVE_filtered
# it is because PM25_rural_annual (the original iteration in line 220) automatically filters out years with all NAs
# can test na.omit on this too.
# test2 <- na.omit(PM2_5_1999_2018_rural_clipped)
test1 <- PM25_rural_annual #fewer columns than IMPROVE_filtered
test1$ID<- paste(PM25_rural_annual$SiteCode,
                 PM25_rural_annual$Year)

anti_join(test1 %>% dplyr::select(ID), IMPROVE_filtered %>% dplyr::select(ID))
difference <- anti_join(IMPROVE_filtered %>% dplyr::select(ID), test1 %>% dplyr::select(ID))
PM25_rural_annual %>% filter(SiteCode == "ATLA1")
...
# Filter out 1999 values and values for HI and AL
...
# 8/15/20 Compare the two data frames for PM25_rural_locations to see why removing NAs decreases final df row count by over 100.
comparison <- anti_join(PM25_rural_locations_more, PM25_rural_locations_less)
comparison
view(PM25_rural_locations_less)
```
