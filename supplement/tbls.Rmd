...
rural_nativePM25 <- inner_join(nativePM25_df, covariates, by = "County") %>%
  subset(as.numeric(as.character(land_use)) >= 4)
rural_generalPM25 <- inner_join(generalPM25_df, covariates, by = "County") %>%
  subset(as.numeric(as.character(land_use)) >= 4)
```

###2.1.2 Create data tables to make bar graphs
```{r}
...
tribal_sensitivity <- rgdal::readOGR("any_tribal_overlap.shp")

#map
leaflet() %>%
  addProviderTiles("CartoDB.PositronNoLabels") %>%
  addPolygons(data = tribal_sensitivity_spatial, color = "#3DACF0")
...
###2.3.4 Monitor coverage for all AI/non-AI counties 
```{r}
...
all_PM25_plot_sens <- ggplot(all_PM25_tbl_sens, 
  ...
  theme(plot.title = element_text(size = 20),
        ...
        legend.title = element_text(size = 14),
        legend.text = element_text(size = 12)) +
  labs(title = expression(paste(PM[2.5], " Monitor Distribution, 2000-2018 (Sensitivity Analysis)")),
       x = "Average number of monitors per year by county type",
       y = "Percent of Total Counties",
       fill = "County Type") 
all_PM25_plot_sens 
...
reservation_PM25_plot_sens <- ggplot(reservation_PM25_tbl_sens, 
  ...
  theme(plot.title = element_text(size = 16),
        ...
        legend.title = element_text(size = 12),
        legend.text = element_text(size = 11)) +
  labs(title = "PM2.5 Monitor Distribution in Counties containing AI Reservations \n vs. Non-AI Counties (Sensitivity Analysis)",
       x = "Average number of annual monitors per county type from 1999-2018",
       y = "Proportion of Counties",
       fill = "County Type")
sum(reservation_PM25_freq$prop)
...
##3.1.1 Re-read in Model PM2.5 data (need to run this chunk as a whole due to overwriting variable name)
```{r}
...
ruralPM25_counties <- main_analysis_counties %>% inner_join(covariates) %>% subset(as.numeric(as.character(land_use)) >=4)
...
summary(detrend_lmer_crude(rural_model_lmer))
summary(detrend_lmer_partial(rural_model_lmer))
