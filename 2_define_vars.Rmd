...
### 2.1.2 Define Census >5% AI/AN Counties
```{r}
...
```
```{r median HH income in AI}
native_census_hhinc <- get_decennial(geography = "county", 
                               variables = native_pop_var, 
                               year = 2010)
...
### 2.1.3 Define Tribal Entities Counties
```{r}
...
# I conducted an overlap analysis in QGIS by importing the tribal48 shapefile and the county shapefile and running the overlap function to join the layers (input layer = counties), which spits out a temporary layer showing the percent overlap of reservations in each county, for each county in the attribute table (I believe the column is called overlap_pc. I went into the properties of this layer, filtered out those with overlap_pc >= 20%, and exported this as a shapefile. I then read this in below:
...
# 9/03/21 edit 46113 to 46102 to not drop counties
covariates = covariates %>% mutate(County = str_replace(County, "46113", "46102"))

## 9/29/21 save out as a df
write_csv(covariates,
          "intermediate/covariates.csv")
```
...
#### 2.4.2 Extract monitor PM2.5 concentrations for non-AI populated counties, for diff classification groups separately
